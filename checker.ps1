...
# FIXED: All syntax errors, Clean Defender, Real Monitor First Connect
# ========================================================
...
try {
    ...
    if ($CoreIsolation.Enabled -eq 1) {
        ...
        Write-Log " Result: DISABLED" -Color Red
        Add-SecurityIssue "Core Isolation" "HIGH"
    }
} catch {
    Write-Log " Result: DISABLED" -Color Red
...
# ========================================================
# SECTION E: MONITOR INFORMATION (CLEAN + REAL FIRST CONNECT)
# ========================================================
...
try {
    ...
    foreach ($Mon in $WmiMonitors) {
        ...
        $InstanceParts = $Mon.InstanceName -split '\\'
        $RegPath = if ($InstanceParts.Count -ge 2) { "HKLM:\SYSTEM\CurrentControlSet\Enum\DISPLAY\$($InstanceParts[1])" } else { $null }
        $FirstSeen = if ($RegPath) { Get-RegistryKeyLastWriteTime $RegPath } else { $null }
        $LastSeen = if ($IsActive) { Get-Date } else { $FirstSeen }
        ...
        if ($FirstSeen) {
            $FirstAgo = (Get-Date) - $FirstSeen
            Write-Log "First Connected: $($FirstSeen.ToString('yyyy-MM-dd HH:mm:ss')) ($([math]::Floor($FirstAgo.TotalDays)) days ago)" -Color Cyan
        } else {
            Write-Log "First Connected: Unknown" -Color Yellow
        }

        if ($LastSeen) {
            $LastAgo = (Get-Date) - $LastSeen
            Write-Log "Last Connected: $($LastSeen.ToString('yyyy-MM-dd HH:mm:ss')) ($([math]::Floor($LastAgo.TotalDays)) days ago)" -Color Yellow
        }
...
# ========================================================
# SECTION H: WINDOWS DEFENDER CONFIGURATION (CLEAN)
# ========================================================
