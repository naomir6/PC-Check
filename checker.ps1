...
# FINAL: Possible Macro Software | No Test-Path -or Error
# ========================================================
...
# ========================================================
# SECTION D: SUSPICIOUS FILE DETECTION (UNSIGNED + CHEATS)
# ========================================================
...
if (Test-Path $prefetchPath) {
    ...
    foreach ($pf in $pfFiles) {
        $execName = $pf.Name -replace '-[A-F0-9]{8}\.pf$', ''
...
# ========================================================
# SOFTWARE CHECK: Logitech G HUB / Razer Synapse (FIXED)
# ========================================================
if (Test-Path "$env:PROGRAMFILES\LGHUB\lghub.exe") {
    $global:SoftwareDetected += "Logitech G HUB"
}

# Razer Synapse 3
if (Test-Path "$env:PROGRAMFILES\Razer\Synapse3") {
    $synapse3Files = Get-ChildItem "$env:PROGRAMFILES\Razer\Synapse3" -Filter "RzDev_*.exe" -ErrorAction SilentlyContinue
    if ($synapse3Files) {
        $global:SoftwareDetected += "Razer Synapse"
    }
}
# Razer Synapse Legacy
...
Write-Log "XIM Matrix Device: $(if($global:DetectionResults.Location -contains 'USB Device'){'DETECTED'}else{'NOT DETECTED'})" -Color $(if($global:DetectionResults.Location -contains 'USB Device'){'Red'}else{'Green'})
