...
# FINAL: USB/PCIe BEFORE LOGON | No Duplicates | No Errors
# ========================================================
...
$global:PreLogonDevices = @()

# === SAVE TO DOWNLOADS ===
$DownloadsPath = "$env:USERPROFILE\Downloads"
if (-not (Test-Path $DownloadsPath)) {
    try { New-Item -Path $DownloadsPath -ItemType Directory -Force | Out-Null } catch { exit }
}

$OutputFile = "$DownloadsPath\SystemScan_$(Get-Date -Format 'yyyyMMdd_HHmmss').txt"
try { $null = New-Item -Path $OutputFile -ItemType File -Force -ErrorAction Stop } catch { exit }
...
try {
    ...
    $UserLogonTime = $LogonEvent.TimeCreated
} catch {
    $OS = Get-CimInstance -ClassName Win32_OperatingSystem
    $UserLogonTime = $OS.LastBootUpTime
    Write-Log " WARNING: Using boot time as fallback for logon time" -Color Yellow
}
...
Write-Log " User Logon Time: $($UserLogonTime.ToString('yyyy-MM-dd HH:mm:ss'))" -Color Cyan
Write-Log "" -NoTimestamp
...
# ========================================================
# SECTION I: USB/PCIe DEVICES CONNECTED BEFORE LOGON
# ========================================================
Write-Log "SECTION I: USB/PCIe DEVICES CONNECTED BEFORE LOGON" -NoTimestamp -Color Yellow
Write-Log "============================================================" -NoTimestamp -Color Yellow

$PreLogonCount = 0
try {
    $Events = Get-WinEvent -LogName "System" -ErrorAction SilentlyContinue | 
             Where-Object { $_.Id -in @(20001, 20003, 21001, 21003) -or $_.Message -match "USB|PCI" } |
             Sort-Object TimeCreated -Descending

    foreach ($Event in $Events) {
        $EventTime = $Event.TimeCreated
        if ($EventTime -lt $UserLogonTime) {
            $DeviceName = ($Event.Message -split '\n' | Select-String -Pattern "Device Name|FriendlyName" | Select-Object -First 1).ToString().Trim()
            if (-not $DeviceName) { $DeviceName = "Unknown Device" }
            $PreLogonCount++
            Write-Log " [PRE-LOGON] $DeviceName" -Color Red
            Write-Log "     First Seen: $($EventTime.ToString('yyyy-MM-dd HH:mm:ss'))" -Color Red
            Write-Log "     Event ID: $($Event.Id) | Provider: $($Event.ProviderName)" -Color Red
            $global:PreLogonDevices += "$DeviceName (Event $($Event.Id))"
            if ($PreLogonCount -ge 10) { break }
        }
    }
} catch { }

if ($PreLogonCount -eq 0) {
    Write-Log " Result: NO DEVICES DETECTED BEFORE LOGON" -Color Green
} else {
    Write-Log " Result: $PreLogonCount DEVICE(S) WERE ACTIVE BEFORE YOU LOGGED IN" -Color Red
}
Write-Log "" -NoTimestamp
...
try {
    ...
    if ($threats) {
        foreach ($t in $threats) {
            Write-Log " Detection Time: $($t.DetectionTime)" -Color Gray
            Write-Log " Threat Name: $($t.ThreatName)" -Color Gray
            Write-Log " Path: $($t.InitialDetectionPath)" -Color Gray
            Write-Log " Action: $($t.ActionStatus)" -Color Gray
            Write-Log "---" -NoTimestamp
        }
    } else {
...
Write-Log "Pre-Logon Devices: $(if($PreLogonCount -gt 0){$PreLogonCount}else{'None'})" -Color $(if($PreLogonCount -gt 0){'Red'}else{'Green'})
