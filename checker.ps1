...
# Fixed: No variable conflicts, clean output
...
# Part 1: All Connected USB/HID Devices
...
# Part 2: XIM Matrix Specific Detection
Write-Log "[2/3] Scanning for XIM Matrix device (VEN:$VendorID PID:$DeviceID)..." -NoTimestamp
try {
    $XimDetected = $false
...
try {
    ...
    if (Test-Path $USBEnumPath) {
        ...
        # Also check for "xim" in friendly names
        foreach ($Key in $USBKeys) {
            try {
...
foreach ($Location in $ScanPaths) {
    ...
    # 1. Pattern Matches (including dapper.dll)
...
$recentExecuted = @()
$recentThreshold = (Get-Date).AddDays(-30)  # Last 30 days

# Prefetch for executed EXEs
$prefetchPath = "C:\Windows\Prefetch"
if (Test-Path $prefetchPath) {
    $prefetchFiles = Get-ChildItem -Path $prefetchPath -Filter "*.pf" -ErrorAction SilentlyContinue | Where-Object { $_.LastWriteTime -gt $recentThreshold } | Sort-Object LastWriteTime -Descending | Select-Object -First 50
...
try {
    ...
    if ($pref.ExclusionPath) {
        ...
        foreach ($ex in $pref.ExclusionPath) {
            Write-Log "  - $ex" -Color Gray
        }
    } else {
        Write-Log " No path exclusions" -Color Green
    }
    if ($pref.ExclusionProcess) {
        Write-Log " Processes:" -Color Cyan
        foreach ($ex in $pref.ExclusionProcess) {
            Write-Log "  - $ex" -Color Gray
        }
    } else {
        Write-Log " No process exclusions" -Color Green
    }
    if ($pref.ExclusionExtension) {
        Write-Log " Extensions:" -Color Cyan
        foreach ($ex in $pref.ExclusionExtension) {
            Write-Log "  - $ex" -Color Gray
        }
    } else {
        Write-Log " No extension exclusions" -Color Green
    }
    if ($pref.ExclusionIpAddress) {
        Write-Log " IP Addresses:" -Color Cyan
        foreach ($ex in $pref.ExclusionIpAddress) {
            Write-Log "  - $ex" -Color Gray
        }
    } else {
        Write-Log " No IP exclusions" -Color Green
    }
} catch {
    Write-Log " ERROR: Could not retrieve Defender preferences - $($_.Exception.Message)" -Color Red
}
Write-Log "" -NoTimestamp
...
try {
    ...
    Write-Log " ERROR: Could not retrieve Defender history - $($_.Exception.Message)" -Color Red
}
Write-Log "" -NoTimestamp
